...
namespace RealisticWorkplacesAndHouseholds
{
    ...
    public class Setting : ModSetting
    {
        ...
        [SettingsUISection(CityServicesSection, PowerPlantGroup)]
        [SettingsUIDisableByCondition(typeof(Setting), nameof(disable_powerplant))]
        public bool powerplant_use_employees_per_gw { get; set; }

        // Helper: disable the MW slider when the toggle is OFF
        [SettingsUIHidden]
        public bool disable_powerplant_employees_per_gw_slider => !powerplant_use_employees_per_gw || disable_powerplant;

        [SettingsUISlider(min = 0.01f, max = 1f, step = 0.01f, scalarMultiplier = 1, unit = Unit.kFloatTwoFractions)]
        [SettingsUISection(CityServicesSection, PowerPlantGroup)]
        [SettingsUIDisableByCondition(typeof(Setting), nameof(disable_powerplant_employees_per_gw_slider))]
        public float powerplant_employees_per_gw { get; set; }
    ...
    public class LocaleEN : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_vacancy_rate)), $"Percentage of apartments or houses that will be free and reserved for the local population of the city. (Default: 5)" },


                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.commercial_avg_floor_height)), "Average Floor Height (meters)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_avg_floor_height)), $"Average Floor Height for commercial, office, and city services buildings. (Default: 3.05)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_units_per_elevator)), $"Number of Apartments per Elevator. The Elevator area will be subtracted and reduce the space available for apartments in the building. (Default: 70)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_hallway_space)), "Percentage of floor space for hallways" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_hallway_space)), $"Percentage of floor space for hallways. Does not include space for elevators and stairs. (Default: 10)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.disable_row_homes_apt_per_floor)), "Disable Apt. per floors for Row Homes" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_row_homes_apt_per_floor)), $"If disabled, Row Homes will use the average sqm per apartment defined above. Otherwise, a specific number of apartments per floors will be used for Row Homes. (Default: not selected)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rowhomes_apt_per_floor)), "Number of apartments per floor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rowhomes_apt_per_floor)), $"Number of apartments per floor in Row Homes. (Default: 1)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_high_level_less_apt)), $"Number of apartments will be reduced once the building reaches levels 4 and 5. WARNING: This feature may cause performance issues and more homeless people in bigger cities. (Default: not selected)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_l4_reduction)), "Level 4: Apartment Size Increase" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_l4_reduction)), $"Increase in apartment size when compared to the average apartment size. (Default: 10)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_l5_reduction)), "Level 5: Apartment Size Increase" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_l5_reduction)), $"Increase in apartment size when compared to the average apartment size. (Default: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rowhomes_basement)), "Basement" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rowhomes_basement)), $"If set to true, Row Homes will have an extra floor in the basement. (Default: selected)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.industry_avg_floor_height)), "Average Floor Height (meters)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.industry_avg_floor_height)), $"Average Floor Height for industry buildings. (Default: 4.5)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.industry_area_base)), "Large Area Threshold" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.industry_area_base)), $"For industry buildings that have a larger area than the defined threshold, a reduction factor will be applied to reduce the number of workplaces calculated. (Default: 7000)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.police_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.police_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.fire_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.fire_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.garbage_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.garbage_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 95)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.depot_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.depot_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 65)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.port_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.port_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 120)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.commercial_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 45)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker_supermarket)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 65)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker_rec_entertainment)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 93)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.powerplant_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 200)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.increase_power_production)), "Increase Power Production" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.increase_power_production)), $"Increases the amount of electricity produced based on the increase of employees compared to the vanilla game. If there is a decrease of employees, power production will not change. (Default: not selected)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.solarpowerplant_reduction_factor)), "Solar Employee Reduction Factor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.solarpowerplant_reduction_factor)), $"Reduce the calculated worker count for Solar Power Plants by the specified factor. (Default: 5)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.hospital_sqm_per_worker)), "Hospital: Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.hospital_sqm_per_worker)), $"Hospital: Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 100)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.clinic_sqm_per_worker)), "Clinic: Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.clinic_sqm_per_worker)), $"Clinic: Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 110)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.transit_station_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 40)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.transit_station_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.admin_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 25)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.admin_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.airport_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 55)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.airport_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.postoffice_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 46)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.postoffice_sqm_per_worker)), "Square Meters per Worker" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.hospital_sqm_per_patient)), $"Hospital: Number of square meters per patient. Higher numbers will decrease the hospital capacity. (Default: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.hospital_sqm_per_patient)), "Hospital: Square Meters per Patient" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.clinic_sqm_per_patient)), $"Clinic: Number of square meters per patient. Higher numbers will decrease the clinic capacity. (Default: 35)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_elevators_per_sqm)), $"The total amount of space required to have one elevator. This area will be used to calculate the number of elevators in the building, and the elevator area will reduce the available space for workers. (Default: 4180)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_elevators_per_sqm)), "Square Meters per Elevator" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_sqm_per_worker)), $"Number of square meters per worker. Higher numbers will decrease the number of workers. (Default: 23)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_non_usable_space)), $"Percentage of Non-usable Area" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_non_usable_space)), "Percentage of area in Office Buildings that are non-usable. This includes for example: hallways, conference rooms and mail rooms. It does not include space for elevators and stairs. (Default: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_height_base)), $"High Building Floor Threshold" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_height_base)), "For buildings that have higher number of floors as defined with this threshold a reduction factor will be applied to reduce the number of workplaces calculated. (Default: 60)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.students_per_teacher)), "Number of Students per Teacher" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.students_per_teacher)), $"Number of Students per Teacher. This will be used to calculate the number of workers required at schools. (Default: 14)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.support_staff)), "Percentage of Support Staff" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.support_staff)), $"Percentage of staff at schools that are not teachers. (Default: 30)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_per_student)), "Base Square Meters per Student" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_per_student)), $"Space required for each student. This will be used to calculate the school capacity. (Default: 7)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_college_adjuster)), "Extra College Space Factor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_college_adjuster)), $"This factor will be applied to the base square meters per student for Colleges. (Default: 3)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_univ_adjuster)), "Extra University Space Factor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_univ_adjuster)), $"This factor will be applied to the base square meters per student for Universities. (Default: 4)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.service_upkeep_reduction)), "Service Upkeep Cost Reduction" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.service_upkeep_reduction)), $"Reduce the cost of service upkeep. Use this to compensate for the extra cost due to more workers at service buildings. (Default: 70)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.electricity_consumption_reduction)), "Electricity Consumption Reduction" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.electricity_consumption_reduction)), $"Reduce electricity consumptions. Use this to compensate for the increase of electricity consumption in buildings. Electricity consumption is directly related to number of residents or employees in a building, so this mod will cause buildings to use more electricity. (Default: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.water_consumption_reduction)), "Water Consumption Reduction" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.water_consumption_reduction)), $"Reduce water consumptions. Use this to compensate for the increase of water consumption in buildings. Water consumption is directly related to number of residents or employees in a building, so this mod will cause buildings to use more water. (Default: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.noise_factor)), "Noise Factor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.noise_factor)), "Scales noise produced by buildings. 100% = vanilla noise. 0% = no noise. Example: 25% means buildings produce 25% of the vanilla noise (75% reduction)." },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rent_discount)), "Residential Rent Discount" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rent_discount)), $"This mod may cause higher rents due to changes in the number of building households. Use this setting to set a rent discount that will compensate the increase in rent. (Default: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.results_reduction)), "Global Workplace/Households reduction" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.results_reduction)), $"Reduce all workplace and househols results by this percentage. Note that some buildings need to have a minimum of 1 worker or household and those might not be reduced. (Default: 0)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.prison_sqm_per_prisoner)), "Square Meters per Prisoner" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prison_sqm_per_prisoner)), $"Number of square meters per prisoner. Higher numbers will decrease the number of prisoners. (Default: 4)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.prisoners_per_officer)), "Number of Prisoners per Correctional Officer" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prisoners_per_officer)), $"Number of Prisoners per Correctional Officer. This will be used to calculate the number of workers required at prisons. (Default: 4)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prison_non_usable_space)), "Percentage of area in Prison Buildings that are as living quarters for prisoners. This include hallways, cafeterias, medical facilities, offices, etc. (Default: 40)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.evicted_reset_type)), "Select what action to do with evicted households. They can either look for a new home or be deleted. (Default: ResetType.FindNewHome)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.disable_households_calculations)), $"Disable Household Calculations" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_households_calculations)), "Disable calculations for households. Household values for residential buildings will be set to vanilla. Requires restarting the game. (Default: not selected)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.disable_workplace_calculations)), $"Disable Workplace Calculations" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_workplace_calculations)), "Disable calculations for workplaces on Office, Commercial, and Industrial zones. Workplace values will be set to vanilla. Requires restarting the game. (Default: not selected)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_cityservices_calculations)), "Disable calculations for workplaces on City Services buildings. Workplace values will be set to vanilla. Requires restarting the game. (Default: not selected)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.pack_choice)), $"Change asset pack factors settings (Default: PacksEnum.France)" },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.powerplant_use_employees_per_gw)), "Use Employees per GW" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.powerplant_use_employees_per_gw)), "If selected, power plant worker counts will be calculated using Employees per GW (option below). If not selected, workers are calculated using Square Meters per Worker." },
    ...
    public class LocalePT : IDictionarySource
    {
        ...
        public IEnumerable<KeyValuePair<string, string>> ReadEntries(IList<IDictionaryEntryError> errors, Dictionary<string, int> indexCounts)
        {
            return new Dictionary<string, string>
            {
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_avg_floor_height)), $"Altura média de um andar para comércio, escritórios, e serviços públicos. (Padrão: 3.05)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_units_per_elevator)), $"Número de apartamentos por elevador. A área do elevador será subtraída e reduzirá o espaço disponível para apartamentos no edifício. (Padrão: 70)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_hallway_space)), "Porcentagem de área do piso para corredores" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_hallway_space)), $"Porcentagem de espaço do piso para corredores. Não inclui espaço para elevadores e escadas. (Padrão: 10)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_row_homes_apt_per_floor)), $"Se desabilitado, casas geminadas usará a média de m² por apartamento definida acima. Caso contrário, um número específico de apartamentos por andar será usado. (Padrão: não selecionado)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rowhomes_apt_per_floor)), "Número de apartamentos por andar" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rowhomes_apt_per_floor)), $"Número de apartamentos por andar para casas geminadas (Padrão: 1)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_high_level_less_apt)), $"O número de apartamentos será reduzido quando o edifício atingir os níveis 4 e 5. AVISO: Esse recurso pode causar problemas de desempenho e mais moradores de rua em cidades grandes. (Padrão: não selecionado)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_l4_reduction)), "Nível 4: Aumento do tamanho do apartamento" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_l4_reduction)), $"Aumento do tamanho do apartamento em comparação ao tamanho médio. (Padrão: 10)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.residential_l5_reduction)), "Nível 5: Aumento do tamanho do apartamento" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.residential_l5_reduction)), $"Aumento do tamanho do apartamento em comparação ao tamanho médio. (Padrão: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rowhomes_basement)), "Porão" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rowhomes_basement)), $"Se definido como verdadeiro, casas geminadas terão um andar extra no porão. (Padrão: selecionado)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.industry_avg_floor_height)), "Altura média de um andar (metros)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.industry_avg_floor_height)), $"Altura média de um andar para edifícios industriais. (Padrão: 4.5)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.industry_area_base)), "Limite de Grande Área" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.industry_area_base)), $"Para edifícios industriais que tenham uma área maior que o limite definido, será aplicado um fator de redução para reduzir o número de locais de trabalho calculados. (Padrão: 7000)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.police_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.police_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.fire_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.fire_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.garbage_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.garbage_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 95)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.depot_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.depot_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 65)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.port_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.port_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 120)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.commercial_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 45)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker_supermarket)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 65)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.commercial_sqm_per_worker_restaurants)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 37)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.powerplant_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 200)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.increase_power_production)), "Aumentar a produção de energia" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.increase_power_production)), $"Aumenta a quantidade de eletricidade produzida com base no aumento de funcionários em comparação ao jogo vanilla. Se houver uma diminuição de funcionários, a produção de energia não mudará. (Padrão: não selecionado)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.solarpowerplant_reduction_factor)), $"Reduz a contagem calculada de trabalhadores para Usinas de Energia Solar pelo fator especificado. (Padrão: 5)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.hospital_sqm_per_worker)), "Hospital: Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.hospital_sqm_per_worker)), $"Hospital: Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 100)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.clinic_sqm_per_worker)), "Clinica: Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.clinic_sqm_per_worker)), $"Clinica: Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 110)" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.transit_station_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 40)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.transit_station_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.admin_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 25)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.admin_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.airport_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 55)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.airport_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.postoffice_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 46)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.postoffice_sqm_per_worker)), "Metros quadrados por trabalhador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.hospital_sqm_per_patient)), $"Hospital: Número de metros quadrados por paciente. Números maiores diminuirão a capacidade do hospital. (Padrão: 50)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.hospital_sqm_per_patient)), "Hospital: Metros quadrados por paciente" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.clinic_sqm_per_patient)), $"Clinica: Número de metros quadrados por paciente. Números maiores diminuirão a capacidade do clinica. (Padrão: 35)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_elevators_per_sqm)), $"A quantidade total de espaço necessária para ter um elevador. Esta área será usada para calcular o número de elevadores no edifício, e a área do elevador reduzirá o espaço disponível para os trabalhadores. (Padrão: 4180)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_elevators_per_sqm)), "Metros quadrados por Elevador" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_sqm_per_worker)), $"Número de metros quadrados por trabalhador. Números maiores diminuirão o número de trabalhadores. (Padrão: 23)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_non_usable_space)), $"Porcentagem de área não utilizável" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_non_usable_space)), "Porcentagem de área em Edifícios de Escritórios que não são utilizáveis. Isso inclui, por exemplo: corredores, salas de conferência e salas de correspondência. Não inclui espaço para elevadores e escadas. (Padrão: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.office_height_base)), $"Limite de piso de edifício alto" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.office_height_base)), "Para edifícios com um número maior de andares, conforme definido com este limite, será aplicado um fator de redução para reduzir o número de locais de trabalho calculados. (Padrão: 60)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.students_per_teacher)), "Número de alunos por professor" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.students_per_teacher)), $"Número de Alunos por Professor. Isso será usado para calcular o número de trabalhadores necessários nas escolas. (Padrão: 14)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.support_staff)), "Porcentagem de equipe de suporte" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.support_staff)), $"Porcentagem de funcionários em escolas que não são professores. (Padrão: 30)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_per_student)), "Metros quadrados base por aluno" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_per_student)), $"Espaço necessário para cada aluno. Isso será usado para calcular a capacidade da escola. (Padrão: 7)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_college_adjuster)), "Fator de espaço extra para faculdades" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_college_adjuster)), $"Este fator será aplicado metros quadrados base por aluno para faculdades. (Padrão: 3)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.sqm_univ_adjuster)), "Fator de espaço extra para universidades" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.sqm_univ_adjuster)), $"Este fator será aplicado metros quadrados base por aluno para universidades. (Padrão: 4)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.service_upkeep_reduction)), $"Reduza o custo de manutenção de serviços públicos. Use isso para compensar o custo extra devido a mais trabalhadores em prédios de serviços públicos. (Padrão: 70)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.electricity_consumption_reduction)), "Redução do Consumo de Eletricidade" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.electricity_consumption_reduction)), $"Reduza o consumo de eletricidade. Use isso para compensar o aumento do consumo de eletricidade em prédios. O consumo de eletricidade está diretamente relacionado ao número de moradores ou funcionários em um prédio, então este mod fará com que os prédios usem mais eletricidade. (Padrão: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.water_consumption_reduction)), "Redução do Consumo de Água" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.water_consumption_reduction)), $"Reduza o consumo de água. Use isso para compensar o aumento do consumo de água em prédios. O consumo de água está diretamente relacionado ao número de moradores ou funcionários em um prédio, então este mod fará com que os prédios usem mais água. (Padrão: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.noise_factor)), "Fator de Ruído" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.noise_factor)), "Escala o ruído produzido por edifícios. 100% = ruído do jogo original. 0% = sem ruído. Exemplo: 25% significa que os edifícios produzem 25% do ruído do jogo original (redução de 75%)." },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.rent_discount)), "Desconto de Aluguel Residencial" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.rent_discount)), $"Este mod pode causar aluguéis mais altos devido a mudanças no número de domicílios no prédio. Use esta configuração para definir um desconto de aluguel que compensará o aumento do aluguel. (Padrão: 20)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.results_reduction)), "Redução global do local de trabalho/domicílios" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.results_reduction)), $"Reduza todos os resultados de local de trabalho e domicílios por esta porcentagem. Observe que alguns edifícios precisam ter no mínimo 1 trabalhador ou domicílio e estes podem não ser reduzidos. (Padrão: 0)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.prison_sqm_per_prisoner)), "Metros quadrados por prisioneiro" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prison_sqm_per_prisoner)), $"Número de metros quadrados por prisioneiro. Números maiores diminuirão o número de prisioneiros. (Padrão: 4)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.prisoners_per_officer)), "Número de presos por agente penitenciário" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prisoners_per_officer)), $"Número de Prisioneiros por agente penitenciário. Isso será usado para calcular o número de trabalhadores necessários nas prisões. (Padrão: 4)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.prison_non_usable_space)), "Porcentagem de área em penitenciárias que não são usadas como alojamentos para prisioneiros. Isso inclui corredores, refeitórios, instalações médicas, escritórios, etc. (Padrão: 40)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.evicted_reset_type)), "Selecione qual ação fazer com as famílias despejadas de sua residência. Elas podem procurar um novo lar ou serem apagadas. (Padrão: ResetType.FindNewHome)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_households_calculations)), "Desabilitar cálculos para domicílios. Os valores de domicílios para prédios residenciais serão definidos como vanilla. Requer reiniciar o jogo. (Padrão: não selecionado)" },
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.disable_workplace_calculations)), $"Desativar cálculos do locais de trabalho" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_workplace_calculations)), "Desabilite cálculos para locais de trabalho em zonas de escritório, comércio e indústria. Os valores de locais de trabalho serão definidos como vanilla. Requer reiniciar o jogo. (Padrão: não selecionado)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.disable_cityservices_calculations)), "Desabilite cálculos para locais de trabalho em prédios de Serviços Públicos. Os valores dos locais de trabalho serão definidos como vanilla. Requer reiniciar o jogo. (Padrão: não selecionado)" },
                ...
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.pack_choice)), "Alterar as configurações dos fatores dos pacotes de assets (Padrão: PacksEnum.France)" },
                ...
                { m_Setting.GetOptionLabelLocaleID(nameof(Setting.powerplant_use_employees_per_gw)), "Usar Funcionários por GW" },
                { m_Setting.GetOptionDescLocaleID(nameof(Setting.powerplant_use_employees_per_gw)), "Se selecionado, o número de trabalhadores das usinas será calculado usando Funcionários por GW (opção abaixo). Se não selecionado, os trabalhadores são calculados usando Metros quadrados por trabalhador." },
